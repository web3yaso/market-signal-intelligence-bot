...
def numbers(text):
    ...
    return {float(value) * {'': 1, 'k': 1000, 'w': 10000, '万': 10000}[suffix.lower()]
            for value, suffix in re.findall(r'(?<![\d.])(\d+(?:\.\d+)?)\s*([kKwW万]?)', text)}


def normalize(o: Observation, message: Message, context: list[Message], config: dict) -> Observation:
    ...
    if o.resource:
        o.resource.provider = aliases.get(o.resource.provider.casefold(), o.resource.provider.casefold())
    ...
    scoped = o.resource and o.resource.provider == config['scope']['provider'] and o.resource.kind == config['scope']['kind']
    if scoped:
        used = False
        if o.resource.terms_id is None and channel.get('terms_id_for_openai_api_credits'):
            o.resource.terms_id = channel['terms_id_for_openai_api_credits']
            used = True
        if o.quantity and o.quantity.unit is None and channel.get('quantity_unit_default'):
            o.quantity.unit = channel['quantity_unit_default']; used = True
        if o.price:
            if o.price.currency is None and channel.get('price_currency_default'):
                o.price.currency = channel['price_currency_default']; used = True
            if o.price.unit is None and channel.get('price_unit_default'):
                o.price.unit = channel['price_unit_default']; used = True
        if used:
    ...
    if o.action not in ('ignore', 'unresolved'):
        ...
        evidence_text = '\n'.join(visible[src.key()].text for src in o.source_message_ids)
        # Guard the only normalized dimensions we aggregate; absent evidence => null.
        if o.quantity and o.quantity.unit == 'USD_FACE' and not channel.get('quantity_unit_default'):
        ...
        if o.price:
            ...
            if o.price.unit == 'CNY_PER_USD_FACE' and not channel.get('price_unit_default'):
                if not re.search(r'一刀|每.*(?:美元|美金)|per.*(?:USD|dollar)', evidence_text, re.I):
                    o.price.unit = None
    return o
...
class EventStore:
    ...
    def target(self, ref: Ref, resource=None) -> Event:
        matches = [e for e in self.events.values()
                   if ref in e.supporting_message_ids and (resource is None or e.resource == resource)]
        if len(matches) != 1:
    ...
    def apply(self, message: Message, candidates: list[Observation], context: list[Message]):
        ...
        try:
            ...
            if message.forwarded_origin:
                ...
                return accepted, [{'operation': 'link_evidence', 'event_id': source.event_id}], None
            for o in accepted:
                if o.action in ('ignore', 'unresolved') or o.assertion_mode == 'hearsay':
                    effects.append({'operation': 'none', 'reason': o.reason or o.unresolved_reason or o.assertion_mode})
                    continue
                if o.resource is None:
                    raise Rejected('resource_missing')
                scope = self.config['scope']
                if (o.resource.provider, o.resource.kind) != (scope['provider'], scope['kind']):
                ...
                if o.action in ('supply', 'demand'):
                    ...
                    if o.target_message:
                        ...
                        if target.owner == message.sender_id and target.side == o.action:
                            if target.status != 'active':
                                raise Rejected('target_inactive')
                            if (o.quantity is not None and o.quantity != target.quantity) or (o.price is not None and o.price != target.price):
                                raise Rejected('reaffirmation_conflicts_use_explicit_update')
                            linked = target
                    if linked:
                        self.link(linked, message)
                        effects.append({'operation': 'link_evidence', 'event_id': linked.event_id})
                    else:
                        material = repr((message.key(), o.action, o.resource.model_dump()))
                        eid = hashlib.sha256(material.encode()).hexdigest()[:16]
                        if eid in self.events:
                            raise Rejected('duplicate_candidate_for_same_resource')
                        self.events[eid] = Event(event_id=eid, owner=message.sender_id,
                            origin_message=message.ref(), side=o.action, resource=o.resource,
                            quantity=o.quantity, price=o.price, supporting_message_ids=[message.ref()])
                ...
                if o.action == 'set_remaining':
                    ...
                    if o.quantity.kind != 'stock' or target.quantity.kind != 'stock' or o.quantity.unit != target.quantity.unit:
                        raise Rejected('remaining_unit_mismatch')
                    target.quantity = o.quantity
                elif o.action == 'reprice':
                    if not o.price or not target.price or o.price.currency is None or o.price.unit is None:
                        raise Rejected('reprice_unit_missing')
                    if (o.price.currency, o.price.unit) != (target.price.currency, target.price.unit):
                        raise Rejected('reprice_unit_mismatch')
                    if target.side == 'supply' and (o.price.is_soft or o.price.operator != 'eq'):
    ...
    def aggregate(self):
        ...
        excluded = {'supply_quantity_missing_or_unit_unknown': 0,
                    'demand_quantity_missing_or_unit_unknown': 0,
                    'flow_quantity': 0, 'seller_price_missing_or_not_comparable': 0}
        ...
        for event in active:
            ...
            if event.side == 'supply':
                if p and p.operator == 'eq' and not p.is_soft and p.currency == 'CNY' and p.unit == 'CNY_PER_USD_FACE':
                    asks.append(Decimal(str(p.value)))
                else:
                    excluded['seller_price_missing_or_not_comparable'] += 1
        return {'observed_supply_usd_face': amounts['supply'], 'observed_demand_usd_face': amounts['demand'],
                'seller_ask_median_cny_per_usd_face': float(median(asks)) if asks else None,
                'seller_ask_sample_count': len(asks), 'active_event_count': len(active), 'excluded': excluded,
                'noncomparable_terms_count': noncomparable}

    def snapshot(self, message: Message, scenario_id: str, status: str):
        snap = {'scenario_id': scenario_id, 'step': len(self.snapshots) + 1,
                'after_message': message.ref().model_dump(), 'as_of': message.timestamp.isoformat(),
                'processing_status': status, 'events': [e.model_dump(mode='json') for e in self.events.values()],
                'metrics': self.aggregate()}
        self.snapshots.append(copy.deepcopy(snap))
