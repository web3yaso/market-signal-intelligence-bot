...
def normalize_state(events):
    ...
    for event in events:
        ...
        event["supporting_message_ids"] = sorted(
            event["supporting_message_ids"],
            key=lambda r: (r["channel_id"], r["message_id"]),
        )
        normalized.append(event)
    return sorted(
        normalized,
        key=lambda e: json.dumps(
            [e["origin_message"], e["side"], e["resource"], e["quantity"], e["price"]],
            sort_keys=True,
        ),
    )


def score_observations(expected: list[dict], actual: list[dict]) -> tuple:
    ...
    weights = [
        [
            100 * (expected_observation["action"] == actual_observation["action"])
            + 10
            * (
                expected_observation.get("resource")
                == actual_observation.get("resource")
            )
            + sum(
                expected_observation.get(field_name)
                == actual_observation.get(field_name)
                for field_name in ("quantity", "price", "target_message")
            )
            for actual_observation in actual
        ]
        for expected_observation in expected
    ]
    best, best_pairs = -1, []
    match_count = min(len(expected), len(actual))
    if match_count:
        for expected_indices in itertools.combinations(
            range(len(expected)), match_count
        ):
            for actual_indices in itertools.permutations(
                range(len(actual)), match_count
            ):
                score = sum(
                    weights[expected_index][actual_index]
                    for expected_index, actual_index in zip(
                        expected_indices, actual_indices
                    )
                )
                if score > best:
                    best, best_pairs = score, list(
                        zip(expected_indices, actual_indices)
                    )
    pairs = best_pairs
    counts = Counter()
    fields = Counter()
    ...
    for expected_index, actual_index in pairs:
        ...
        missing.remove(expected_index)
        remaining.remove(actual_index)
        counts["decisions_total"] += 1
        if expected_observation["action"] == actual_observation["action"]:
            counts["decisions_correct"] += 1
            if expected_observation["action"] in SIGNALS:
                counts["true_positive_signals"] += 1
        else:
            if expected_observation["action"] in SIGNALS:
                counts["false_negative_signals"] += 1
            if actual_observation["action"] in SIGNALS:
                counts["false_positive_signals"] += 1
            errors.append(
                {
                    "expected_action": expected_observation["action"],
                    "actual_action": actual_observation["action"],
                }
            )
        # Field totals include nulls, incorrect actions and abstentions. No denominator cherry-picking.
        for field_name in FIELDS:
            expected_fields, actual_fields = leaves(
                expected_observation.get(field_name), field_name
            ), leaves(actual_observation.get(field_name), field_name)
            for path in expected_fields:
                counts["fields_total"] += 1
                fields[path + ":total"] += 1
                if (
                    path in actual_fields
                    and actual_fields[path] == expected_fields[path]
                ):
                    ...
                    errors.append(
                        {
                            ...
                            "actual": actual_fields.get(path),
                        }
                    )
    for expected_index in missing:
        expected_observation = expected[expected_index]
        counts["decisions_total"] += 1
        if expected_observation["action"] in SIGNALS:
            counts["false_negative_signals"] += 1
        for field_name in FIELDS:
            for path in leaves(expected_observation.get(field_name), field_name):
                counts["fields_total"] += 1
                fields[path + ":total"] += 1
        errors.append({"missing_action": expected_observation["action"]})
    for actual_index in remaining:
        actual_observation = actual[actual_index]
        counts["extra_outputs"] += 1
        if actual_observation["action"] in SIGNALS:
            counts["false_positive_signals"] += 1
        errors.append({"extra_action": actual_observation["action"]})
...
def evaluate_pipelines(pipelines, gold, expected_snapshots, mode):
    ...
    for p in pipelines:
        ...
        for snap in p.store.snapshots:
            ...
            state_ok = normalize_state(snap["events"]) == normalize_state(
                wanted["events"]
            )
            metrics_ok = all(
                snap["metrics"].get(k) == v for k, v in wanted["metrics"].items()
            )
                ...
                failures.append(
                    {
                        ...
                        "step": snap["step"],
                        "kind": "state_or_aggregate",
                        "state_matches": state_ok,
                        "metrics_match": metrics_ok,
                        "expected_metrics": wanted["metrics"],
    ...
    return {
        "mode": mode,
        "prompt_id": PROMPT_ID if mode == "api" else None,
        "extraction_metrics": extraction,
        "state_metrics": {
            k: totals[k]
            for k in ("snapshots_total", "states_correct", "aggregates_correct")
        },
        "scenarios_passed": all_scenarios_pass,
        "scenarios_total": len(pipelines),
        "runs": summaries,
        "failures": failures,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run per-scenario evaluation with explicit API/fixture mode."
    )
    ...
    try:
        ...
        extractor = (
            APIExtractor()
            if args.mode == "api"
            else FixtureExtractor(ROOT / "dataset/gold.jsonl")
        )
        selected = [
            s
        ...
        pipelines = []
        for s in selected:
            p = Pipeline(extractor, config, s["scenario_id"]).run(
                replay(ROOT / "dataset/messages.jsonl", s["channel_id"])
            )
            save_run(p, args.out / s["scenario_id"])
            pipelines.append(p)
        result = evaluate_pipelines(
            pipelines,
            read_jsonl(ROOT / "dataset/gold.jsonl"),
            read_jsonl(ROOT / "dataset/expected_snapshots.jsonl"),
            args.mode,
        )
        result["split"] = args.split
        result["dataset_sha256"] = hashlib.sha256(
            (ROOT / "dataset/messages.jsonl").read_bytes()
        ).hexdigest()
        ...
        lines = [
            "# Evaluation",
            f"Mode: {args.mode}; split: {args.split}",
            ...
            (
                "LLM metrics: not measured (fixture replay)"
                if args.mode == "fixture"
                else "LLM metrics: see evaluation.json"
            ),
