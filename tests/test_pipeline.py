...
def obs(message):
    return Extraction.model_validate({'observations':GOLD_MAP[message.key()]['observations']}).observations
...
class PipelineTests(unittest.TestCase):
    def test_all_golden_transitions(self):
        ...
        runs = [Pipeline(fixture(), CONFIG, s['scenario_id']).run(messages(s['channel_id'])) for s in manifest['scenarios']]
        result = evaluate_pipelines(runs, GOLD, read_jsonl(DATA / 'expected_snapshots.jsonl'), 'fixture')
    ...
    def test_failed_message_not_retried(self):
        extractor = StubExtractor(Result(status='failed',reason='timeout'))
    ...
    def test_atomic_rollback_on_second_invalid_candidate(self):
        ...
        bad.action='retract';bad.target_message=m.ref()
        ...
        self.assertIsNotNone(reason);self.assertEqual(effects,[]);self.assertFalse(store.events)

    def test_fabricated_evidence_rejected(self):
        m=messages()[0];o=obs(m)[0];o.evidence_quote='unseen facts'
        store=EventStore(CONFIG)
    ...
    def test_fabricated_number_rejected(self):
        m=messages()[0];o=obs(m)[0];o.quantity.value=999999
        store=EventStore(CONFIG)
        self.assertEqual(store.apply(m,[o],[])[2],'number_not_in_current_message')

    def test_conflicting_mutations_rollback(self):
        root=messages()[0];update=messages()[5].model_copy(update={'text':'改成 3.6，或者 3.9。'})
        first=obs(messages()[5])[0];second=first.model_copy(deep=True)
        first.evidence_quote=update.text;second.evidence_quote=update.text;second.price.value=3.9
        store=EventStore(CONFIG);store.apply(root,obs(root),[])
    ...
    def test_soft_seller_reprice_is_not_confirmed(self):
        root=messages()[0];update=messages()[5];candidate=obs(update)[0];candidate.price.is_soft=True
        store=EventStore(CONFIG);store.apply(root,obs(root),[])
        self.assertEqual(store.apply(update,[candidate],[root])[2],'seller_reprice_not_confirmed_point')
        self.assertEqual(store.aggregate()['seller_ask_median_cny_per_usd_face'],3.8)

    def test_missing_currency_unit_not_hallucinated(self):
        m=messages('wx_units')[0];o=obs(m)[0]
        o.price.currency='CNY';o.price.unit='CNY_PER_USD_FACE';o.quantity.unit='USD_FACE'
        store=EventStore(CONFIG);normalized,_,reason=store.apply(m,[o],[])
    ...
    def test_forward_does_not_trust_model_seller(self):
        store=EventStore(CONFIG);root,forward=messages()[:2]
        store.apply(root,obs(root),[])
        candidate=obs(forward)[0];candidate.action='supply';candidate.assertion_mode='direct'
        store.apply(forward,[candidate],[root])
        self.assertEqual(len(store.events),1)
        self.assertEqual(next(iter(store.events.values())).owner,'alice')

    def test_forward_missing_source_rejected(self):
        m=messages()[1]
        store=EventStore(CONFIG)
        _,_,reason=store.apply(m,obs(m),[messages()[0]])
        self.assertEqual(reason,'target_not_unique_or_missing')
        self.assertFalse(store.events)
    ...
    def test_unknown_terms_not_aggregated(self):
        m=messages()[0];o=obs(m)[0];o.resource.terms_id='some_other_terms'
        store=EventStore(CONFIG);store.apply(m,[o],[])
    ...
    def test_repeated_offer_by_same_owner_without_link_is_distinct(self):
        p=Pipeline(fixture(),CONFIG).run(messages()[:1]);m=messages()[2].model_copy(update={'sender_id':'alice'})
        p.process(m)
    ...
    def test_out_of_order_rejected_without_model_call(self):
        ...
        self.assertEqual(extractor.calls,1)
        self.assertEqual(p.records[-1]['reason'],'out_of_order_not_supported')

    def test_context_two_hops_and_no_nearest_neighbor(self):
        ms=messages('wx_reply');index=MessageIndex()
        for m in ms:index.accept(m)
    ...
    def test_context_never_uses_future_or_loops(self):
        ...
        index=MessageIndex();index.accept(following)
    ...
    def test_contract_rejects_naive_time_and_bad_numbers(self):
        data=messages()[0].model_dump(mode='json');data['timestamp']='2026-09-15T09:00:00'
        with self.assertRaises(ValidationError):Message.model_validate(data)
        for value in [float('nan'),float('inf'),-1,True]:
            with self.assertRaises(ValidationError):Quantity(value=value)

    def test_report_escapes_untrusted_html(self):
        m=messages()[0].model_copy(update={'text':'</script><script>alert(1)</script>'})
        p=Pipeline(StubExtractor(Result(status='ok')),CONFIG).run([m])
        with tempfile.TemporaryDirectory() as temp:
            path=Path(temp)/'report.html';render_report(p,path);html=path.read_text()
    ...
    def test_all_null_price_does_not_block_remaining_update(self):
        ...
        candidate['price'] = dict(value=None, currency=None, unit=None, operator=None, is_soft=None)
    ...
    def test_partial_or_unknown_price_fields_are_not_discarded(self):
        for price in ({'value': None, 'currency': 'CNY'}, {'value': None, 'unexpected': None}, {}):
    ...
    def test_report_windows_are_anchored_to_sample_end(self):
        snapshots=[{'as_of':'2020-01-01T00:00:00+00:00'},
...
class APITests(unittest.TestCase):
    def client(self, transport, count=100):
        return APIExtractor(model='test-model',key='test-key',base_url='https://api.example.invalid/v1',
                            transport=transport,counter=lambda *_:(count,'test_counter'))

    def test_api_response_is_validated_and_request_has_no_gold(self):
        m=messages()[0];payload={'observations':GOLD_MAP[m.key()]['observations']};requests=[]
        def transport(body):
        ...
        self.assertEqual(result.status,'ok');self.assertEqual(len(requests),1)
    ...
    def test_budget_blocks_request(self):
        called=[]
        result=self.client(lambda body:called.append(body),3001).extract(messages()[0],[],CONFIG)
        self.assertEqual(result.status,'unresolved');self.assertFalse(result.called);self.assertFalse(called)

    def test_timeout_no_retry(self):
        calls=[]
        def transport(body):calls.append(body);raise TimeoutError()
    ...
    def test_invalid_json_or_truncation_cannot_mutate(self):
        for text,finish in [('not json','stop'),('{"observations": []}','length')]:
            client=self.client(lambda body:{'choices':[{'message':{'content':text},'finish_reason':finish}]})
            p=Pipeline(client,CONFIG).run(messages()[:1])
            self.assertEqual(p.records[0]['status'],'failed');self.assertFalse(p.store.events)

    def test_cost_is_not_zero_when_unknown(self):
        with patch.dict('os.environ',{'INPUT_PRICE_PER_MILLION':'','OUTPUT_PRICE_PER_MILLION':''}):
            c=self.client(lambda body:{'choices':[{'message':{'content':'{"observations": []}'},'finish_reason':'stop'}],
                                       'usage':{'prompt_tokens':100,'completion_tokens':40}})
            result=c.extract(messages()[0],[],CONFIG)
        self.assertEqual(result.status,'ok');self.assertIsNone(result.estimated_cost)

    def test_env_is_not_executed(self):
        import os
        with tempfile.TemporaryDirectory() as temp,patch.dict(os.environ,{},clear=True):
            p=Path(temp)/'.env';p.write_text('DEMO_VAR=$(touch NEVER)\n')
            load_env(p);self.assertEqual(os.environ['DEMO_VAR'],'$(touch NEVER)')
    ...
    def test_abstention_and_extra_signals_are_scored(self):
        ...
        self.assertGreater(counts['fields_total'],0);self.assertEqual(counts['fields_correct'],0)
        counts,_,_=score_observations([],expected)
    ...
    def test_failed_call_is_not_correct_negative_classification(self):
        p=Pipeline(StubExtractor(Result(status='failed',reason='timeout')),CONFIG,'S5').run(messages('wx_mixed'))
        result=evaluate_pipelines([p],GOLD,read_jsonl(DATA/'expected_snapshots.jsonl'),'api')
    ...
    def test_guard_rejection_is_not_correct_negative_classification(self):
        ...
        p = Pipeline(StubExtractor(Result(status='ok', observations=[candidate])), CONFIG, 'S5').run([message])
    ...
    def test_valid_model_abstention_is_not_guard_failure(self):
        ...
        p = Pipeline(StubExtractor(Result(status='ok', observations=obs(message))), CONFIG, 'S5').run([message])
